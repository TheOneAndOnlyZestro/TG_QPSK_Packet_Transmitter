...
def demodulate_qpsk(complex_data: np.ndarray, baud_rate: int, sample_rate: int, start_bits: str):
...
def demodulate_bpsk(complex_data: np.ndarray, baud_rate: int, sample_rate: int, start_bits: str):
    N = len(complex_data)
    burst_2 = complex_data**2 
    fft_freqs = np.fft.fftfreq(N, d=1/sample_rate)
    f_offset = fft_freqs[np.argmax(np.abs(np.fft.fft(burst_2)))] / 2.0
    derotated = complex_data * np.exp(-1j * 2 * np.pi * f_offset * (np.arange(N) / sample_rate))
    
    for offset in range(baud_rate):
        syms = derotated[offset :: baud_rate]
        if len(syms) < 2: continue
        diff_phases = np.angle(syms[1:] * np.conj(syms[:-1]))
        bit_str = "".join(['0' if -np.pi/2 <= dp < np.pi/2 else '1' for dp in diff_phases])
        idx = bit_str.find(start_bits)
        if idx != -1: return bit_str[idx + len(start_bits):]
    return None
